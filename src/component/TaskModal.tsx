...
const TaskModal: React.FC<TaskModalProps> = ({ isOpen, onClose, onApply, onSkip, userId }) => {
  ...
  // Detect the auto-generated summary like: "I am working on 1 task of Project." / "... tasks of ..."
  const isAutoGeneratedSummary = (text: string): boolean => {
    const t = (text || '').trim();
    return /^I am working on \d+\s+tasks?\s+of\s+.+\.$/i.test(t);
  };
  ...
  const renderInsights = (
    ...
    return (
      <div className="mt-4 p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
        ...
        {/* Breakpoints & Estimated Time */}
        <div className="mb-4">
          <div className="text-xs font-semibold text-gray-600 mb-2">Breakpoints & Estimated Time</div>
  ...
  const analyzeTaskWithAI = async (taskTitle: string, taskId: string, retryCount = 0) => {
    if (!taskTitle.trim()) {
      setAiAnalyses(prev => ({
        ...prev,
        [taskId]: {
          summary: 'please provide a task',
          subtasks: [],
          totalTime: 0,
          isLoading: false
        }
  ...
  const triggerAnalysis = (taskId: string, title: string, retryCount = 0) => {
    if (title.trim()) {
      if (retryCount < 2) {
        ...
        console.error('Max retries reached for task:', title);
        setAiAnalyses(prev => ({
          ...prev,
          [taskId]: {
